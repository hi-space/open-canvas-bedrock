...
# Generate artifact prompt (used in generate_artifact_node)
GENERATE_ARTIFACT_PROMPT = """You are an AI assistant tasked with generating a new artifact based on the user's request.
Ensure you use markdown syntax when appropriate, as the text you generate will be rendered in markdown.
...
# Reply to general input prompt (used in reply_to_general_input_node)
REPLY_TO_GENERAL_INPUT_PROMPT = """You are an AI assistant tasked with responding to the users question.
...
# Custom action prompts (used in custom_action_node)
CUSTOM_ACTION_REFLECTIONS_PROMPT = """You also have the following reflections on style guidelines and general memories/facts about the user:
<reflections>
{reflections}
</reflections>"""

CUSTOM_ACTION_PREFIX_PROMPT = """You are an AI assistant. The user has provided custom instructions for you to follow.
{custom_instructions}"""
